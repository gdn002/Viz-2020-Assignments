...
void StreamlineIntegrator::process() {
    ...
    if (propSeedMode.get() == 0) {
		...
		if (propForwardDirection) EulerLoop(vectorField, startingPoint, mesh, vertices);
		if (propBackwardDirection) EulerLoop(vectorField, startingPoint, mesh, vertices, true);
